...
func (r *Repository) SaveEmployee(
	...
	_, err := r.db.Exec(ctx, query, identifier, fullname, shortname, position, email, phone, isAdmin)
	if err != nil {
...
func (r *Repository) GetEmployeeByID(ctx context.Context, identifier int) (models.Employee, error) {
	...
	query := `SELECT id, fullname, shortname, position, email, phone, is_admin FROM employees WHERE id=$1`

	err := r.db.QueryRow(ctx, query, identifier).Scan(
		&result.ID, &result.FullName, &result.ShortName, &result.Position, &result.Email, &result.Phone, &result.IsAdmin)
	if err != nil {
